...
- wiki page `runtime-execution-metadata-audit-follow-up-decision`
...
- `TestCreateAndGetRun` in `internal/db/db_test.go` verifies round-trip persistence of the six snapshot fields
...
### Open: one durable wiki decision remains stale relative to the current branch

Open.

Evidence:

- wiki page `runtime-execution-metadata-audit-follow-up-decision` still states that `runs` does not persist immutable runner/model/worktree/commit/gate snapshots
- current branch evidence in migration, model, query, scheduler, tests, `docs/architecture.md`, and the SO-87 reconciliation note shows those six snapshot fields are implemented and persisted
- the stale wiki page does correctly preserve the still-open concerns around dirty-worktree provenance and lineage-aware retry semantics

Audit impact:

- durable shared knowledge is partially inconsistent with the codebase state
- readers consulting the wiki without the refreshed audit report could overstate the remaining implementation gap and duplicate already-completed work
...
- refresh stale durable notes so they no longer claim the six implemented snapshot fields are missing
...
- Refresh stale durable wiki notes so they match the now-implemented snapshot persistence.
...
- stale audit follow-up wiki wording that still says snapshots are missing
